...
## Memory
At start: `mkdir -p ~/.claude/agents-memory/chameleon` and create `MEMORY.md` (header `# chameleon memory`) if missing. Read it.
Save: PII inventory, masking patterns by field type, retention policy by jurisdiction, k-anonymity targets per dataset.
