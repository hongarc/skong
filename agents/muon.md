...
## Memory
At start: `mkdir -p ~/.claude/agents-memory/muon` and create `MEMORY.md` (header `# muon memory`) if missing. Read it.
Save: project's test runner + config, flaky tests with reason, coverage targets.
